...
| [src/render/format.ts](../src/render/format.ts) | Pure formatters/escapers: `escapeHtml`/`escapeAttr`, `formatMastheadDate`, `formatTimestamp`, `sectionSlug`, `titleCase`, `bylineFor`, `storyPageUrl` + `buildXIntentUrl` (ADR-0009), `hashString` (deterministic rank-0 article placement) |
