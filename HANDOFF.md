...
- **Priscilla preamble bug (this morning's cycle):** her `opinion-priscilla-2026-07-14` headline was a
  ...
  "Guard Your Casseroles". **Latent fragility to watch:** the piece parser takes line 1 as the title
  with no preamble guard, so a model that prepends chatter corrupts the headline. If it recurs,
  harden the parser (strip a leading meta-preamble / a `---` HR before the real bold title) rather
  than relying on regeneration. Manifest backed up to the session scratchpad before the delete.
