...
- **Dual Claude generator.** One `Generator` interface, two impls selected by config: subscription (`claude -p --output-format json`, auth via `CLAUDE_CODE_OAUTH_TOKEN` from `claude setup-token`) and API key (Messages API, `ANTHROPIC_API_KEY`). Default to subscription for personal use.
