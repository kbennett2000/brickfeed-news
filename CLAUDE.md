...
- **Pluggable generator.** One `Generator` interface, multiple impls selected by config: the keyless subscription CLIs `grok-terminal` (drives the `grok` CLI headlessly) and `claude` (`claude -p --output-format json`, auth via `CLAUDE_CODE_OAUTH_TOKEN` from `claude setup-token`), plus the `grok` xAI API path and an `apikey` Messages-API stub. **The effective production default is now the keyless `grok-terminal` path** (per ADR-0007) for both text and image — no generation API key needed. See `docs/CONFIGURATION.md` for the full provider matrix.
