...
import type { ClaudeRunner, Generator, GrokChatRunner } from "../types.js";
 ...
 * "claude" provider is the subscription generator (`claude -p`); "apikey" is the
 * Slice 2b Messages-API stub. Custom runners can be injected (used by tests);
 * production leaves them undefined so the real HTTP/CLI boundary is used.
 */
export function createGenerator(
  config: Config,
  opts: { runner?: ClaudeRunner; grokRunner?: GrokChatRunner } = {},
): Generator {
  const { provider, model, grok } = config.generator;
